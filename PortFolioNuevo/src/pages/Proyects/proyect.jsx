...
import Captiview from '../../../public/assets/Captiview.png';
    ...
    <div className="container mx-auto p-2 mt-40 mb-40">
      <div className="text-center mb-8">
        <h2 className="text-4xl font-bold text-center text-green-400 mb-10">
          {translations.proyect.title}
        </h2>
      </div>
      {proyectosAMostrar.map((proyecto) => (
        <div
          key={proyecto.id}
          className={`mb-8 shadow-lg p-5 border rounded-md ${lightMode ? 'bg-black bg-opacity-80' : 'bg-white'}`}
        >
          <Image
            src={proyecto.img}
            alt={proyecto.img}
          ...
          />
          <h3
            className={`text-2xl sm:text-3xl m-5 text-center font-semibold animated animate__fadeIn ${lightMode ? 'text-green-400 ' : 'text-green-800 '}`}
          >
            {proyecto.titulo}
          </h3>
          <p
            className={`mt-4 mx-auto w-4/5 text-sm sm:text-lg text-center ${expandedProyectos[proyecto.id] ? 'animated animate__fadeIn' : ''} ${lightMode ? 'text-gray-300' : 'text-gray-600'} `}
          >
            ...
            {proyecto.descripcion.length > 150 && (
              <span className="flex justify-center mt-5">
                ...
                >
                  {expandedProyectos[proyecto.id] ? (
                    <p className="text-neutral hover:text-red-600 transition duration-200 underline">
                      {translations.proyect.readLess}
                    </p>
                  ) : (
                    translations.proyect.readMore
                  )}
                </button>
              </span>
            )}
