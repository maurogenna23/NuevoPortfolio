...
const Home = () => {
  ...
  return (
    <main
      className={`flex flex-col justify-center h-full ${lightMode ? 'bg-gray-100' : ''}`}
    >
      <section
        id="home"
        className={`flex flex-col lg:flex-row ${lightMode ? 'bg-gray-300' : 'bg-zinc-900'}`}
      >
        ...
        <div className="flex flex-col lg:flex-row rounded-lg mb-4 md:mb-44 items-center">
          <div className="w-full xl:w-1/2 p-4 md:p-24 mt-6 xl:text-left">
            <h3
              className={`font-semibold text-xl sm:text-xl mb-2 sm:mb-5 mt-10 ${lightMode ? 'text-green-600' : 'text-green-400'}`}
            >
              {translations.home.hello}
            </h3>
            <h2
              className={`font-semibold text-2xl sm:text-5xl mb-2 sm:mb-5  mt-2 sm:mt-5 ${lightMode ? 'text-neutral-900' : 'text-gray-200'}`}
            >
              {translations.home.welcome}
            </h2>
            <p
              className={`mb-4 sm:mb-6 font-medium ${lightMode ? 'text-neutral-900' : 'text-gray-200'}`}
            >
      ...
      <hr
        className={`w-4/5 mx-auto ${
          lightMode
            ? 'border-t-2 border-gray-700 bg-gray-800'
      ...
      <div className="hidden lg:block">
        {showBackToTop && (
          <button
            className="fixed bottom-3 z-40 right-5 p-2 bg-blue-500 text-white rounded-full cursor-pointer transition duration-300 animated animate__fadeIn hover:bg-blue-700"
